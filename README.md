...
- **Shareable room links** — a "Copy invite link" / share button on the lobby and in-game screens hands out `/room/CODE` directly (using the native share sheet on mobile where available), and sharing that link in Slack/Discord/iMessage/etc. shows the site's branded preview image (see [Link previews](#link-previews--og-image)).
...
This is deliberately a static, site-wide image rather than one baked with each room's invite code — that would need a server-side render per room (link-unfurling bots don't execute JS), which for this app would mean Firebase Cloud Functions, which in turn requires the paid Blaze plan. A room link still works exactly the same for actually joining; the shared preview just doesn't show that room's code.
...
The host picks a mode when creating a room; it's fixed for that room's lifetime. A separate **Jump-In** toggle (also host-set at creation, off by default) layers on top of either mode: whenever your hand holds a card that matches the discard pile's top card *exactly* — same color and same number/symbol — you can play it immediately, out of turn, any time (as long as nothing else has been played on top of it yet and nothing's currently pending — no stack to resolve, no color/reveal choice in progress). Play then continues from whoever jumped in, skipping straight to the next player after them. [Official rule](https://matteluno.fandom.com/wiki/Jump-In); jump-in-eligible cards in your hand get a distinct cyan glow (versus the yellow "your turn" glow) so you can spot them at a glance, and pressing **X** jumps in with the first eligible card without having to click it precisely (a "Press X to jump in!" hint appears whenever one's available). Not Space — that's the browser's native scroll key.

- **Classic** — standard UNO plus the house rules below.
- **UNO No Mercy** — a from-scratch implementation of Mattel's [UNO Show 'Em No Mercy](https://service.mattel.com/instruction_sheets/HVW18-Eng.pdf) (2023), a genuinely different 168-card game: value-tiered Draw stacking (+2/+4/+6/+10, any Draw card of equal-or-higher value can stack, not just matching types), **Discard All** and **Skip Everyone** action cards, a **7's Swap** (forces a full hand swap with a player of your choice — click their seat at the table to pick who) and **0's Pass** (everyone's hand passes to the next player), **Wild Color Roulette** (the next player picks a color, then reveals cards from the draw pile — visibly, in a shared arch everyone can see, not hidden — until they hit it), and the **Mercy rule**: reach the mercy limit (host-set at room creation, no cap, 25 by default) and you're knocked out for the rest of the round (your cards re-enter the deck on the next reshuffle), with the last player standing winning outright. There's no plain "Wild" card in this deck. Officially none of the four wild cards have a "choose a color" step, but the app adds one **house rule**: playing Wild Reverse Draw 4 / Draw 6 / Draw 10 lets you pick the next color, same as classic's Wild/Wild+4 — Wild Color Roulette is the exception, since its "choose a color" already belongs to the next player (the one hunting for it during the reveal), not whoever plays it. Scoring adds a 250-point bonus per knockout. Every card count here (including the non-uniform ones — Draw Four and Skip Everyone only get 2 copies per color instead of 3; the four wilds split 8/8/4/4, not evenly) was hand-verified off real card photos rather than assumed. Implementation lives in `src/lib/no-mercy/` as a fully parallel rules engine — see [Project structure](#project-structure).
...
- **Mobile-responsive** — the round table, piles, direction arrow, and card sizes all scale down to fit a phone screen (a `sm:` breakpoint at 640px, matched in JS so fly-animations and hand-fan spacing stay in sync with what's actually rendered), and the table gets a taller aspect ratio on small screens so opponent seats have room to fan out without colliding with the header. Your own hand dense-packs to always fit the screen width with zero horizontal scrolling — hovering a card ripples it and its nearest neighbors apart (parted, lifted, enlarged) so individual cards stay pickable out of an arbitrarily large hand. Once a hand passes 40 cards (the Mercy limit has no cap, so this happens), it splits into multiple fans of up to 40 cards each, stacked one under another.
...
scripts/            og-image-svg.mjs (SVG template) + generate-default-og.mjs — regenerates public/og-image.png
