...
- **Draw-until-playable (house rule)** — if you have no legal play, drawing doesn't pass your turn: click the draw pile, and if that card isn't playable either, it's still your turn — click again. Once you draw something playable you may play it or keep it and pass. Each draw is its own click/animation, one card at a time, and stacking penalty draws (below) work the same way — no click-batching, so it feels like actually drawing from a pile rather than an instant top-up. (The unlimited deck guarantees you'll eventually draw something playable.)
