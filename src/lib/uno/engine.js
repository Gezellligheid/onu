 ...
 * whoever it lands on next inherits it, count untouched), or a Reverse
 * (redirects the whole stack back to whoever played it) — everything else,
 * including the color/number matching rules above, is suspended until the
 * stack is resolved one of those ways or drawn. Once you've drawn even one
 * card against the stack (mustFinishDrawing), that window has closed —
 * you're committed to drawing the rest, no more countering.
 */
